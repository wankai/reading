...
希尔伯特的计划失败了，但是极大促进了可计算性理论的研究，很多数学家们都在构建能描述通用计算过程的形式化系统。其中涌现出了三个形式系统：图灵的__图灵机__，克里尼的__递归函数论__（基于希尔伯特1925年的工作），丘奇的__λ演算__。它们都定义了一些基本的操作数，组合这些基本操作数的简单规则，并且都提供了推理系统。
...
图灵证明了一个结论：找不到一个通用的算法来判定某个图灵机是否会终止。这是著名的图灵停机问题，
