...
六十年代，计算机在各领域的应用爆炸式地发展，人们重新思考可计算性理论怎样指导编程实践。1963年，McCarthy受递归函数论和λ演算的影响，给计算这个概念提出了一种数学基础，他的思想最终反应在LISP(LISt Processing)语言中。LISP非常简洁，只基于列表数据结构和递归函数。它是无类型的，并且程序和数据都是用列表表示，所以LISP有着强大的元编程能力。LISP是最早严格形式化的语言之一，但是它不是一个纯函数式语言，也包含一些命令式的成分。
